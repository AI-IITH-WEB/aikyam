...
import { PanelDiscussion1Members } from "../constants";

const PanelDiscussion = ({ title }) => {
  const panelMembers =
    title === "Panel Discussion I" ? PanelDiscussion1Members : [];
  return (
    <div className="p-4 bg-white border-t border-teal-200 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-12 md:px-12 pt-4 pb-10 gap-6">
        {panelMembers?.map((member) => (
          <div
            key={member?.id}
            className="col-span-12 md:col-span-6 rounded-lg border border-solid border-teal-800 overflow-hidden transition-all duration-300 hover:shadow-lg md:hover:scale-[1.02]"
          >
            <AnimatedSection delay={200}>
              <a href={member?.linkendInProfile || member?.homepage} target="_blank" rel="noopener noreferrer">
